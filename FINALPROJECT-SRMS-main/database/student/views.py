...
def faculty_second(request,Year,Group_Given):
	Student_Filter_Year_Of_Admission = Student.objects.filter(Year_Of_Admission = Year, Group = Group_Given).order_by('-Group','Year_Of_Admission').order_by('Roll_Number')
	content = {
		...
		if Student.objects.filter(Roll_Number = request.POST.get('Roll_Number')).exists():
			...
			for key,value in request.POST.items():
				if key == 'Email_Id':
					...
					Roll = value
				elif key == 'Admission_Number':
					Admin = value
				elif key == "First_Name":
					fname = value
				elif key == "Last_Name":
					lname = value
			email_send(email,Roll,Admin,fname,lname)
			if form.is_valid():
				form.save()
				content = {
						'form':form
...
def Delete_Student(request,Year,Group,Roll_Number):
	...
	Student_Filter_Year_Of_Admission = Student.objects.filter(Year_Of_Admission = Year).filter(Group = Group)#.order_by('-Group','Year_Of_Admission')
	#print(Student_Filter_Year_Of_Admission)
	content = {
				'Student_Filter' : Student_Filter_Year_Of_Admission
	}
	return render(request, 'temp/class.html',content)
...
def Teacher_o(request,name_id):
	visit = Teacher_Course.objects.filter(Teacher_Id = name_id)
	content = {
			'visit' : visit,
			'User' : name_id
				}
	...
	if request.POST['SUBMIT'] == 'Mark':
		...
		for i in range(2,len(values)):
			if request.POST[values[i]] == 'on':
				...
				Month = Find_Month(Date[0])
				year = int(Date[2])
				Values = Student_Attendance.objects.filter(Teacher_Id_Attendance = Id).filter(Course_Code = Course).filter(Semester = Semester).filter(Date_Time_Original__date = datetime.date(year,Month,Day))
				Values.delete()
		Rolls = Student.objects.filter(Year_Of_Admission = Year).filter(Group = Group).values_list('Roll_Number')
		Previous = Student_Attendance.objects.filter(Semester = Semester).filter(Course_Code = Course).filter(Roll_Number_Attendance__in = Rolls).order_by('Date_Time_Original')
		Previous_Date = Previous.values_list('Date_Time_Original')
...
def Teacher_fifth(request,Id,Year,Group,Semester,Course,Date):
	D = Date.split(" ")
	Day = int(D[1].split(",")[0])
	Month = Find_Month(D[0])
	year = int(D[2])
	print(Day,Month,Year)
	Values = Student_Attendance.objects.filter(Teacher_Id_Attendance = Id).filter(Course_Code = Course).filter(Semester = Semester).filter(Date_Time_Original__date = datetime.date(year,Month,Day)).order_by('Date_Time_Original')
	content = {
			'Date' : Date[5:7],
			'Month' : Date[0:3],
			'Year' : Date[9:13],
...
def Teacher_seventeen(request,Id,Year,Semester,Course,Group):
	...
	for i in B:
		if len(Previous_Attendance.filter(Roll_Number_Marks = i)) == 0:
			...
			for student in Previous_Attendance:
				if student.Roll_Number_Marks.Roll_Number == i and student.Marks_Alloted != float(A[i]):
					student.Marks_Alloted = float(A[i])
					student.save()
	studentgraph = Student.objects.filter(Group = Group).filter(Year_Of_Admission = Year).order_by('Roll_Number')
	dataframe = Bargraph(Id,Year,Group,Semester,Course)[0]
	average_present = round(Bargraph(Id,Year,Group,Semester,Course)[1],2)
...
def Student_first(request):
	if request.method == 'POST':
		...
		if student.exists() and student[0].valid_User_Password == request.POST.get('Password'):
			...
			return HttpResponse('INVALID DETAILS OR NOT A STUDENT')
def Student_o(request,Roll_Number):
