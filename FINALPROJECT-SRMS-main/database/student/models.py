...
class Student(models.Model):
	...
	valid_User_Password = models.CharField(max_length = 20,editable = False)
	'''def save(self,*args,**kwargs):
		if self.Student_Image:
			image = Img.open(StringIO.StringIO(self.Student_Image.read()))
			image.thumbnail((512*512), Img.ANTIALIAS)
			output = BytesIO()
			image.save(output, format='JPEG', quality=75)
			output.seek(0)
			self.photo= InMemoryUploadedFile(output,'ImageField', "%s.jpg" %self.Student_Image.name, 'image/jpeg', output.len, None)
		super(Mymodel, self).save(*args, **kwargs)
		if self.Student_Image:
			self.Student_Image = get_thumbnail(self.Student_Image,'512x512',format = 'JPEG').url
...
class Student_Marks(models.Model):
	...
	Roll_Number_Marks = models.ForeignKey(Student,on_delete = models.CASCADE, default = '')
	...
	Marks_Alloted = models.DecimalField(max_digits = 6, decimal_places = 2,null = True)

class Student_Attendance(models.Model):
	#Attendance Given
	Date_Time_Marks = models.DateTimeField(auto_now = True)
	Date_Time_Original = models.DateTimeField(default = '')
	Semester = models.CharField(max_length= 20, choices = Semester_Choices, null = True)
	Course_Code = models.ForeignKey(Courses,on_delete = models.PROTECT, default = '')
	Roll_Number_Attendance =  models.ForeignKey(Student,on_delete = models.CASCADE ,default = '')
